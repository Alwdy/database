...
const rules = reactive({
  username: [
    ...
    { min: 5, max: 15, message: '长度应该在5~15个字符之间', trigger: 'blur' },
  ],
  password: [
...
const loginVerify = () => {
  axios.post('/api/base/captcha',{data:{}}).then(res => {
    rules.captcha.push({
      max: res.data['data'].captchaLength,
      min: res.data['data'].captchaLength,
      message: `请输入${res.data['data'].captchaLength}位验证码`,
      trigger: 'blur',
    })
    picPath.value = res.data['data'].picPath
    console.log(res.data['data'].captchaId);
    loginForm.captchaId = res.data['data'].captchaId
  })
}
loginVerify()
const submitForm = async() => {
      await axios.post('/api/base/login',loginForm).then(res => {
      ...
      ElMessage({
        ...
        showClose: true,
         ...
         router.push('/views/DatabaseView')
      }
}
const register = () => {
